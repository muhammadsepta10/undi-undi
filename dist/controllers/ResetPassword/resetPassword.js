...
exports.validateEmail = (req, res) => {
    ...
    db_1.connection.query(`SELECT email FROM profiles WHERE email='${email}'`, (err, data) => {
        if (err) {
            res.send({
          ...
          <html lang="en">
            ...
            <body>
              ...
              <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>
                ...
                db_1.emailSenderConnection.query(`INSERT INTO email_sender VALUES (NULL, 3, 'Reset Password', '${email}', '${message}', 0, NOW(), NOW(), 0, NULL, NULL)`, (err, rows) => {
                    if (err) {
                        res.send({
    ...
    db_1.connection.query(`SELECT email FROM profiles WHERE email='${email}'`, (err, data) => {
        if (err) {
            ...
            if (data.length < 1) {
                ...
                if (data.length > 0) {
                    db_1.connection.query(`UPDATE profiles SET password='${password}' WHERE email='${email}'`, (errr, rows) => {
                        if (errr) {
                            res.send({
                                message: "Update Password Failed",
                                success: 0,
...
exports.resetPasswordProfiles = (req, res) => {
    const email = req.body.email;
    ...
    db_1.connection.query(`SELECT email FROM profiles WHERE email='${email}'`, (err, data) => {
        ...
        if (data.length > 0) {
            db_1.connection.query(`SELECT password from profiles WHERE email='${email}'`, (err, rows) => {
                if (err) {
                    res.send({
                        message: "Update Password Failed",
                        success: 0,
                    ...
                    if (verifyPassword === true) {
                        db_1.connection.query(`UPDATE profiles SET password='${newPassword}' WHERE email='${email}'`, (errr, rows) => {
                            if (errr) {
                                res.send({
                                    message: "Update Password Failed",
                                    success: 0,
