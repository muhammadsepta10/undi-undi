...
export const surveyQuestions = (req: Request, res: Response) => {
              ...
              if (data.length < 1) {
                ...
                connection.query(
                  `SELECT survey_entries.id, survey_entries.profile_id,survey_entries.periode,periode.id as periode,periode.start_date,periode.end_date FROM survey_entries LEFT JOIN periode ON survey_entries.periode = periode.id WHERE survey_entries.profile_id = ${
                    ...
                    "YYYY-MM-DD"
                  )}' AND periode.end_date>'${moment().format("YYYY-MM-DD")}'`,
                  (err: any, data: any) => {
                    if (err) {
                      logger.error({
                        message: "failed to get data survey_entries",
                      ...
                      if (data.length < 1) {
                        connection.query(
                          `SELECT COUNT(*) as total FROM survey_questions JOIN periode ON survey_questions.periode = periode.id WHERE periode.start_date<='${moment().format(
                            "YYYY-MM-DD"
                          )}' AND periode.end_date>'${moment().format(
                            "YYYY-MM-DD"
                          )}' AND periode.category=5`,
                          (err: any, total: any) => {
                            if (err) {
                              logger.error({
                                ...
                                API: API,
                                err,
                                query: `SELECT COUNT(*) as total FROM survey_questions JOIN periode ON survey_questions.periode = periode.id WHERE periode.start_date<='${moment().format(
                                  "YYYY-MM-DD"
                                )}' AND periode.end_date>'${moment().format(
                                  "YYYY-MM-DD"
                                )}' AND periode.category=5`
                                ...
                                const prevPage =
                                  currentPage === 1
                                    ...
                                    : "https://beta-api.undiundi.id/survey/questions" +
                                      ...
                                      (currentPage - 1);
                                connection.query(
                                  `SELECT survey_questions.id,question FROM survey_questions JOIN periode ON survey_questions.periode = periode.id WHERE periode.start_date<='${moment().format(
                                    "YYYY-MM-DD"
                                  )}' AND periode.end_date>'${moment().format(
                                    "YYYY-MM-DD"
                                  )}' AND periode.category=5 ORDER BY survey_questions.id ASC LIMIT ${firstData}, ${dataPerPage}`,
                                  (err: any, data: any) => {
                                    if (err) {
                                      logger.error({
                                        message: "failed to get data",
                                        API: API,
                                        err,
                                        query: `SELECT survey_questions.id,question FROM survey_questions JOIN periode ON survey_questions.periode = periode.id WHERE periode.start_date<='${moment().format(
                                          "YYYY-MM-DD"
                                        )}' AND periode.end_date>'${moment().format(
                                          "YYYY-MM-DD"
                                        )}' AND periode.category=5 ORDER BY survey_questions.id ASC LIMIT ${firstData}, ${dataPerPage}`
...
export const surveyAnswer = (req: Request, res: Response) => {
  ...
  connection.query(
    ...
    (err: any, lang: any) => {
      ...
      if (language === "en") {
        connection.query(
          `SELECT id,code,answer FROM survey_answers WHERE survey_question_id=${req.params.questionsId}`,
          (err: any, data: any) => {
            if (err) {
              logger.error({
                message: "failed to get data",
...
export const surveyEntries = (req: Request, res: Response) => {
              ...
              connection.query(query, (err: any, duplicateEntries: any) => {
                if (err) {
                  ...
                  if (duplicateEntries.length >= 1) {
                    ...
                    const query = `SELECT COUNT(*) AS countQuestions FROM survey_questions LEFT JOIN periode ON survey_questions.periode = periode.id WHERE periode.start_date<='${moment().format(
                      "YYYY-MM-DD"
                    )}' AND periode.end_date>'${moment().format(
                      "YYYY-MM-DD"
                    )}' AND periode.category=5`;
                    connection.query(query, (err: any, count: any) => {
                      if (err) {
                        ...
                        if (count[0] < 1) {
                          ...
                          const query = `SELECT COUNT(*) AS countAnswer FROM survey_entries LEFT JOIN periode ON survey_entries.periode = periode.id WHERE survey_entries.profile_id = ${
                            verify.id
                                      ...
                                      (err: any, checkLead: any) => {
                                        if (err) {
                                          ...
                                          if (checkLead.length < 1) {
                                            const query = `INSERT INTO survey_leaderboard (profile_id,periode,createdAt) VALUES(${
                                              verify.id
                                            },${data[0].id},'${moment().format(
                                              "YYYY-MM-DD"
                                            )}')`;
                                            connection.query(
                                              query,
                                            ...
                                            // insert point
                                            const query = `INSERT INTO point (profile_id,date,type_point,point) VALUES(${
                                              verify.id
                                            },'${moment().format(
                                              "YYYY-MM-DD"
                                            )}',3,10)`;
...
export const userOrder = (req: Request, res: Response) => {
        ...
        connection.query(query, (err: any, data: any) => {
          if (err) {
            ...
            if (data.length >= 1) {
              ...
              connection.query(query, (err: any, row: any) => {
                if (err) {
                  logger.error({
                    message: "error",
                    API: API,
                  ...
                  if (row[0].countLead > 0) {
                    ...
                    connection.query(query, (err: any, prize: any) => {
                      if (err) {
                        logger.error({
                          message: "error",
                          API: API,
